...
export function ConnectionSettings() {
  const { connect, disconnect } = useInkathon()
  const [isLoading, setIsLoading] = useState(false)

  const handleConnect = async () => {
    setIsLoading(true)
    const selectedNetwork = (
      document.getElementById("select-network") as HTMLSelectElement
    )?.value
    const selectedWallet = (
      document.getElementById("select-wallet") as HTMLSelectElement
    )?.value
